...
- [x] T001 Initialize Go module: `go mod init github.com/cyrill/mikrotik-keepalive-server` → `go.mod`
- [x] T002 Add dependencies to `go.mod`: `modernc.org/sqlite`, `github.com/go-chi/chi/v5` — run `go mod tidy`
- [x] T003 [P] Create directory structure: `internal/db/`, `internal/handler/`, `internal/model/`, `web/templates/`, `web/static/`
- [x] T004 [P] Download Chart.js v4 UMD bundle and save to `web/static/chart.umd.min.js`
- [x] T005 [P] Create `web/static/style.css` with minimal dashboard styles (flexbox layout, chart containers, router selector, date picker)
- [x] T006 [P] Create placeholder `web/templates/dashboard.html` (empty HTML shell — content filled in US2)
...
- [x] T007 Create `internal/model/event.go` — define `PingEvent` struct: `ID int64`, `RouterID string`, `ReceivedAt time.Time`, `SourceIP string`
- [x] T008 Create `internal/db/db.go` — `Open(path string) (*sql.DB, error)`: open SQLite file, enable WAL mode (`PRAGMA journal_mode=WAL`), enable foreign keys, return connection
- [x] T009 Create schema migration in `internal/db/db.go` — `Migrate(db *sql.DB) error`: create `ping_events` table and both indexes as defined in `specs/002-stats-dashboard/data-model.md`
- [x] T010 Create `internal/db/cleanup.go` — `DeleteOldEvents(db *sql.DB) error`: delete rows where `received_at < datetime('now', '-7 days')`; run once at startup then every 24h via goroutine
- [x] T011 Create `main.go` skeleton — read `PORT` (default `8080`), `DB_PATH` (default `keepalive.db`), `TZ` env vars; call `db.Open`, `db.Migrate`; startup validation: attempt a test write and delete to confirm DB is writable (exit with descriptive error if not); start cleanup goroutine
- [x] T012 Set up chi router in `main.go` — add `chi.NewRouter()`, attach `middleware.Logger` and `middleware.Recoverer`; mount `/static/` file server from embedded `web/static/` FS
- [x] T013 Add `//go:embed` declarations in `main.go` (or a dedicated `embed.go`): embed `web/static` as `staticFS` and `web/templates` as `templateFS`
- [x] T014 Create `internal/handler/health.go` — `HealthHandler(db *sql.DB) http.HandlerFunc`: run `SELECT 1`, return `{"status":"ok","db":"ok"}` on success or `{"status":"degraded","db":"error: ..."}` with HTTP 503 on failure
- [x] T015 Wire `/health` route in `main.go`; start HTTP server with `http.ListenAndServe`
...
- [x] T016 [US1] Create `StorePing` in `internal/db/queries.go` — `StorePing(db *sql.DB, routerID, sourceIP string) error`: `INSERT INTO ping_events (router_id, source_ip) VALUES (?, ?)` using server-set `received_at` default
- [x] T017 [US1] Create `internal/handler/ping.go` — `PingHandler(db *sql.DB) http.HandlerFunc`: extract `id` query param; validate against regex `^[a-zA-Z0-9_-]{1,64}$`; return 400 if invalid/missing; call `StorePing`; return `200 OK\nOK`; return 500 on storage error; log each attempt with `slog` (router_id, source_ip, outcome)
- [x] T018 [US1] Wire `GET /ping` route in `main.go`
...
- [x] T019 [P] [US2] Create `DailyStats` in `internal/db/queries.go` — `DailyStats(db *sql.DB, routerID, date string) ([]BucketPoint, error)`: 1-minute bucket GROUP BY query for the given day; zero-fill all 1440 minutes in Go before returning
- [x] T020 [P] [US2] Create `ListRouters` in `internal/db/queries.go` — `ListRouters(db *sql.DB) ([]RouterInfo, error)`: SELECT DISTINCT router_id with last_seen, first_seen, total_pings; ORDER BY first_seen ASC
- [x] T021 [US2] Implement `GET /api/routers` handler in `internal/handler/dashboard.go` — call `ListRouters`, return JSON array; return `[]` if empty
- [x] T022 [US2] Implement `GET /api/stats/daily` handler in `internal/handler/dashboard.go` — require `router_id` query param; optional `date` (default today in server TZ); call `DailyStats`; return JSON per contract `specs/002-stats-dashboard/contracts/dashboard-api.md`; return 404 if router unknown
- [x] T023 [US2] Fill `web/templates/dashboard.html` — complete HTML: `<select>` for router, `<input type="date">` limited to today−6d through today, Day/Week toggle buttons, two `<canvas>` elements (daily-chart, weekly-chart), `<div id="last-ping">` for last-ping timestamp display
- [x] T024 [US2] Add vanilla JS in `web/templates/dashboard.html` — on load: fetch `/api/routers`, populate selector, set default router and date; fetch `/api/stats/daily`, render Chart.js bar chart (green=#4caf50 for count>0, grey=#e0e0e0 for count=0); wire date picker `change` event to refetch and re-render; wire Day/Week toggle buttons; update URL query params on state change (`history.replaceState`)
- [x] T025 [US2] Implement `GET /` handler in `internal/handler/dashboard.go` — parse `?router=`, `?date=`, `?view=` query params; render `dashboard.html` template passing params as template data for initial page state; wire in `main.go`
- [x] T026 [US2] Add last-ping timestamp to `GET /api/routers` response and display in dashboard `<div id="last-ping">` via JS
...
- [x] T027 [P] [US3] Create `WeeklyStats` in `internal/db/queries.go` — `WeeklyStats(db *sql.DB, routerID string) ([]DayPoint, error)`: per-day COUNT query for last 7 days; zero-fill all 7 days in Go before returning
- [x] T028 [US3] Implement `GET /api/stats/weekly` handler in `internal/handler/dashboard.go` — require `router_id`; call `WeeklyStats`; return JSON per contract; return 404 if router unknown; wire in `main.go`
- [x] T029 [US3] Add weekly chart JS logic in `web/templates/dashboard.html` — on "Week" toggle click: show weekly-chart canvas, hide daily-chart canvas; fetch `/api/stats/weekly` for current router; render Chart.js bar chart (X=day labels, Y=ping count); add `onClick` handler on bars: set date picker value to clicked day, switch to Day view, refetch daily data
...
- [x] T030 [US4] Update JS in `web/templates/dashboard.html` — wire router `<select>` `change` event: update current router state, refetch and re-render active chart (daily or weekly), update last-ping display, update URL `?router=` param
- [x] T031 [US4] Handle single-router case in JS — when `/api/routers` returns 1 entry, optionally add `disabled` attribute to `<select>` and show router name as text label
- [x] T032 [US4] Handle zero-router case in JS — when `/api/routers` returns `[]`, show full-page empty state ("No routers have pinged yet. Configure your MikroTik router to send pings to this server.") and hide chart area
...
- [x] T033 [P] Create `Dockerfile` — multi-stage build: `golang:1.22-alpine` build stage → `FROM scratch` (or `alpine`) runtime; copy binary only; expose `PORT`; `ENTRYPOINT ["./server"]`
- [x] T034 [P] Add `slog` structured logging to all handlers — each request logs: handler name, router_id (where applicable), status code, duration
- [x] T035 [P] Add `Cache-Control: max-age=86400` header to `/static/` file server for static assets
- [x] T036 [P] Update `CLAUDE.md` between SPECKIT markers to reflect final project structure and `go build`/`go test` commands
- [x] T037 Run `quickstart.md` validation — follow all steps in `specs/002-stats-dashboard/quickstart.md`; verify binary builds, pings stored, dashboard renders, Docker image builds
